...
DEFAULT_DATA_PATH = (
    Path(__file__).resolve().parents[3] / "Dataset" / "vnindex_weekly_sentiment_merged.parquet"
)
...
TARGET_COLUMNS = ["future_ret_1w", "future_ret_4w", "future_ret_4w_compound"]
...
def add_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if article_columns.issubset(out.columns):
        ...
        out["sentiment_intensity"] = out["positive_ratio"] + out["negative_ratio"]

    if "sentiment_index" in out.columns:
        out["sentiment_lag_1w"] = out["sentiment_index"].shift(1)
        out["sentiment_lag_2w"] = out["sentiment_index"].shift(2)
        out["sentiment_lag_4w"] = out["sentiment_index"].shift(4)
        out["sentiment_ma_4w"] = out["sentiment_index"].rolling(window=4, min_periods=4).mean()
        out["sentiment_ma_8w"] = out["sentiment_index"].rolling(window=8, min_periods=8).mean()

    if "weekly_return" in out.columns:
        out["future_ret_4w_compound"] = (
            (1 + out["weekly_return"].shift(-1))
            * (1 + out["weekly_return"].shift(-2))
            * (1 + out["weekly_return"].shift(-3))
            * (1 + out["weekly_return"].shift(-4))
            - 1
        )
        out["return_lag_2w"] = out["weekly_return"].shift(2)
        out["return_lag_4w"] = out["weekly_return"].shift(4)
        out["return_ma_4w"] = out["weekly_return"].rolling(window=4, min_periods=4).mean()
        out["return_ma_8w"] = out["weekly_return"].rolling(window=8, min_periods=8).mean()
        ...
        out["return_vol_8w"] = out["weekly_return"].rolling(window=8, min_periods=8).std()
        out["return_vol_12w"] = out["weekly_return"].rolling(window=12, min_periods=12).std()

    if "vol_total" in out.columns:
        out["vol_change_1w"] = out["vol_total"].pct_change()
    ...
    if {"sentiment_index", "sentiment_index_z"}.issubset(out.columns):
        ...
        out["sentiment_shock_1w"] = out["sentiment_index"] - out["sentiment_index"].shift(1)
        ...
        out["sentiment_shock_vs_ma4"] = (
            safe_divide(out["sentiment_index"] - sentiment_ma4, sentiment_std8)
        )
        out["extreme_negative_sentiment"] = (out["sentiment_index_z"] < -1.5).astype(int)
        out["extreme_positive_sentiment"] = (out["sentiment_index_z"] > 1.5).astype(int)

    if {"negative_ratio", "positive_ratio", "log_article_count"}.issubset(out.columns):
        out["negative_ratio_change_1w"] = out["negative_ratio"] - out["negative_ratio"].shift(1)
        out["positive_ratio_change_1w"] = out["positive_ratio"] - out["positive_ratio"].shift(1)
    ...
    if {
        ...
        out["negative_attention"] = (
            out["negative_ratio_change_1w"].clip(lower=0) * out["news_attention_shock"].clip(lower=0)
        )
        out["positive_attention"] = (
            out["positive_ratio_change_1w"].clip(lower=0) * out["news_attention_shock"].clip(lower=0)
        )
    ...
    if {"log_vol_total", "high_low_range_pct", "weekly_return", "return_vol_12w"}.issubset(out.columns):
        ...
        out["volume_shock_12w"] = (
            safe_divide(out["log_vol_total"] - volume_ma12, volume_std12)
        )
        out["range_shock_12w"] = (
            safe_divide(out["high_low_range_pct"] - range_ma12, range_std12)
        )
        ...
        out["large_up_week"] = (out["weekly_return"] > 0.03).astype(int)
    ...
    if interaction_columns.issubset(out.columns):
        ...
        out["bad_news_after_down_week"] = (
            out["negative_attention"] * (out["weekly_return"] < 0).astype(int)
        )
        out["good_news_after_up_week"] = (
            out["positive_attention"] * (out["weekly_return"] > 0).astype(int)
        )
...
def split_dataframe(
    ...
    for name, split in [("train", train), ("val", val), ("test", test)]:
        sizes[f"{name}_start"] = str(split["week_start"].min().date()) if len(split) else "NA"
        sizes[f"{name}_end"] = str(split["week_end"].max().date()) if len(split) else "NA"
...
def fit_once(
    ...
    prepared = prepare_splits(
        train, val, test, feature_columns, target_column, look_back, scaler
    )
...
def baseline_predictions(
    ...
    for column in ["weekly_return", "return_lag_1w"]:
...
def window_target_dates(split_df: pd.DataFrame, look_back: int) -> pd.Series:
    return pd.to_datetime(split_df["week_end"].iloc[look_back - 1 :], errors="coerce")
...
def plot_return_predictions(
    ...
    all_dates = pd.to_datetime(
        pd.concat([train["week_end"], val["week_end"], test["week_end"]], ignore_index=True),
        errors="coerce",
    )
    ...
    ax.set_title(f"Weekly SVR VNINDEX {target_column} | look_back={run.look_back} | {run.params}")
...
def run_pipeline(args: argparse.Namespace) -> dict[str, object]:
    raw_df = read_weekly_data(args.data)
    ...
    prediction_path = output_dir / "weekly_svr_vnindex_predictions.csv"
    save_predictions(prediction_path, train, val, test, run)

    tuning_path = None
    if args.tune:
        tuning_path = output_dir / "weekly_svr_vnindex_tuning_results.csv"
        tuning_results.to_csv(tuning_path, index=False, encoding="utf-8-sig")

    print("\nWeekly sentiment SVR VNINDEX return prediction")
    print("=" * 90)
    ...
    print(f"Best look-back: {run.look_back} weeks")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run SVR on weekly VNINDEX sentiment and market features."
    )
    parser.add_argument("--data", default=DEFAULT_DATA_PATH, type=Path)
    parser.add_argument("--target", default="future_ret_1w", choices=TARGET_COLUMNS)
    ...
    parser.add_argument("--look-back", default=4, type=int)
    ...
    parser.add_argument("--look-back-grid", default="1,2,4,8,12")
    ...
    parser.add_argument(
        "--output-dir",
        default=Path(__file__).resolve().parent / "weekly_sentiment_outputs",
        type=Path,
    )
    parser.add_argument(
        "--plot-path",
        default="weekly_svr_return_prediction.png",
        help="File name/path for the plot. Omit or set empty to skip.",
    )
...
def main() -> None:
    ...
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    with open(output_dir / "weekly_svr_vnindex_summary.json", "w", encoding="utf-8") as f:
