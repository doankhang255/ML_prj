...
def add_price_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out[TARGET_COLUMN] = out["Close"].shift(-1)
    ...
    return out.dropna(subset=[TARGET_COLUMN, "Target_Date"] + EXOG_COLUMNS).reset_index(drop=True)
...
def plot_predictions(
    ...
    plt.plot(train["Target_Date"], train[TARGET_COLUMN], label="Train target", color="blue")
    plt.plot(val["Target_Date"], val[TARGET_COLUMN], label="Validate target", color="red")
    plt.plot(test["Target_Date"], test[TARGET_COLUMN], label="Test target", color="orange")
    plt.plot(val["Target_Date"], val_pred, label="ValidatePred", color="purple", linestyle="--")
    plt.plot(test["Target_Date"], test_pred, label="Predict", color="green", linestyle="--")
...
def run_one_model(
    ...
    if args.plot_dir:
        output_path = Path(args.plot_dir) / f"{model_name}_{args.split}_prediction_ver2.png"
...
def run_pipeline(args: argparse.Namespace) -> dict[str, dict[str, dict[str, float]]]:
    ...
    print(f"Exogenous features: {EXOG_COLUMNS if args.model != 'sarima' else 'SARIMA uses no exog'}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run leakage-safe SARIMA/SARIMAX next-close prediction on VNINDEX data."
    )
