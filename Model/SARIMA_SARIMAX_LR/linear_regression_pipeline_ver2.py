...
def add_price_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return out.dropna(subset=FEATURE_COLUMNS + [TARGET_COLUMN, "Target_Date"]).reset_index(drop=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run leakage-safe Linear Regression next-close prediction on VNINDEX data."
    )
    ...
    parser.add_argument(
        "--plot-path",
        default=Path(__file__).resolve().parent / "linear_regression_prediction_ver2.png",
        type=Path,
